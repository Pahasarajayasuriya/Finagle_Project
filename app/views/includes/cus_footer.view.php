<!DOCTYPE html>
<html>

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title></title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">


    <link rel="stylesheet" href="<?=ROOT?>/assets/css/cus_footer.css">
</head>

<body>
    <div class="home-section2">
    <section class="footer">
        <div class="box-container">
            <div class="box">
                <h3>quick links</h3>
                <a href="user.php"> <i class="fas fa-arrow-right"></i> Home</a>
                <a href="product.php"> <i class="fas fa-arrow-right"></i>Products</a>
                <a href="user.php#aboutus"> <i class="fas fa-arrow-right"></i>About</a>
                <a href="user.php#review"> <i class="fas fa-arrow-right"></i> Review</a>
                <a href="export.php"> <i class="fas fa-arrow-right"></i>Exports</a>
                <a href="user.php#contactus"> <i class="fas fa-arrow-right"></i>Contact</a>
            </div>

            <div class="box">
                <h3>products</h3>
                <a href="product.php"> <i class="fas fa-arrow-right"></i> Bread & Buns </a>
                <a href="product.php"> <i class="fas fa-arrow-right"></i> Cakes </a>
                <a href="product.php"> <i class="fas fa-arrow-right"></i> Frozen Foods </a>
            </div>

            <div class="box">
                <h3>branches</h3>
                <a href="#"> <i class="fas fa-arrow-right"></i> Pelawatta </a>
                <a href="#"> <i class="fas fa-arrow-right"></i> Ja-Ela </a>
                <a href="#"> <i class="fas fa-arrow-right"></i> Nugegoda </a>
                <a href="#"> <i class="fas fa-arrow-right"></i> Jawatta - Colombo 05 </a>
                <a href="#"> <i class="fas fa-arrow-right"></i> Kollupitiya - Colombo 03 </a>
                <a href="#"> <i class="fas fa-arrow-right"></i> Dehiwala </a>
                <a href="#"> <i class="fas fa-arrow-right"></i> Borella </a>
            </div>

            <div class="box">
                <h3>follow us</h3>
                <a href="https://www.facebook.com/finaglelanka/"> <i class="fab fa-facebook-f"></i> facebook </a>
                <a href="https://www.instagram.com/finagle_lanka/"> <i class="fab fa-instagram"></i> instagram </a>
                <a href="https://www.youtube.com/channel/UCmW9-WuMsWq8peGj7-5ybcA"> <i class="fab fa-youtube"></i> youtube </a>
            </div>
        </div>
    </section>
    <section class="credit">Copyright <i class="fa-regular fa-copyright"></i> Finagle Lanka (Pvt) Ltd || all rights reserved</section>
    </div>
</body>

</html>