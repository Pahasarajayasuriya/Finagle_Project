...
class admin_employeesModel extends Model
{
    ...
    public function pagination($start_from, $limit)
    {
        ...
        $query = "SELECT * FROM `users` WHERE role = 'employee' AND availability_status = '1' LIMIT $start_from, $limit";
        return $this->query($query);
    }

    public function get_count()
    {
        $query = "SELECT COUNT(id) FROM `users` WHERE role = 'employee'AND availability_status = '1'";
        return $this->query($query);
    }
